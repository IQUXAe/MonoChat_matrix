...
Future<AppLocalizations> lookupL10n(Locale locale) async {
...
Future<void> _tryPushHelper(
  ...
  if (client == null) {
    // In background, we need to initialize a client
  ...
  if (client == null) {
    Logs().e('Client could not be initialized');
    return;
  }

  // Set presence to offline for background push to avoid showing user as online
  // when they only received a push notification without opening the app
  final previousSyncPresence = client.syncPresence;
  ...
  // Restore previous sync presence state
  client.syncPresence = previousSyncPresence;

  Logs().v('Push helper has been completed!');
}

class MonoChatPushPayload {
  final String? clientName, roomId, eventId;

  MonoChatPushPayload(this.clientName, this.roomId, this.eventId);

  factory MonoChatPushPayload.fromString(String payload) {
    final parts = payload.split('|');
    if (parts.length != 3) {
      return MonoChatPushPayload(null, null, null);
    }
    return MonoChatPushPayload(parts[0], parts[1], parts[2]);
  }
...
enum MonoChatNotificationActions { markAsRead, reply }
